...
// Firebase auth is handled in firebase-auth.js (loaded as a separate module in index.html).
// app.js listens for the 'dsar-auth-ready' custom event to know it can safely call init().
// It also exposes window._dsarRerender so firebase-auth.js can trigger UI updates on sync.
...
window._dsarRerender = reloadAndRerender;

// Initialize app after Firebase auth confirms a logged-in user
window.addEventListener('dsar-auth-ready', () => {
    init();
});
