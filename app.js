...
// Estate Invite Code — required during account registration
const INVITE_CODE = 'KARUNA-2026';

let currentUser = null;
let activeSnapshotUnsubscribers = [];
let suppressFirestoreWrite = false; // prevent write loops when onSnapshot updates localStorage
let appInitialized = false; // prevent double-init

// ── FIRESTORE SYNC LAYER ──────────────────────────────────────────────────
// Transparently intercept all localStorage.setItem calls for dsar_* keys
// and mirror them to Firestore so all 4 estate managers stay in sync.
...
// Load all shared data from Firestore into localStorage (called on login)
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            if (data.value !== undefined) {
                _origSetItem(docSnap.id, data.value);
            }
        });
        suppressFirestoreWrite = false;
    } catch (err) {
        console.warn('[DSAR] Failed to load from Firestore, using local data:', err);
        suppressFirestoreWrite = false;
    }
}

// Real-time listener for all shared_data documents
    ...
    const unsub = onSnapshot(collection(db, 'shared_data'), (snapshot) => {
        ...
        snapshot.docChanges().forEach(change => {
            if (change.type === 'modified' || change.type === 'added') {
                ...
                const existingValue = localStorage.getItem(change.doc.id);
                if (data.value !== existingValue) {
        ...
        if (needsRerender && appInitialized) {
            reloadAndRerender();
        }
    }, (err) => {
        console.warn('[DSAR] Real-time sync error:', err);
    });
...
function reloadAndRerender() {
    loadProjectData();
    activeMediaIndices = {};
    plantsState.forEach(p => { activeMediaIndices[p.id] = 0; });
...
function setupAuthUI() {
    ...
    // We use window._dsarAuthMode to read the current auth mode.
        ...
        try {
            if (isRegisterMode()) {
                ...
                if (inviteCode !== INVITE_CODE) {
                    throw new Error('Invalid estate invite code. Contact a manager for access.');
                }
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            // Pull latest shared data from Firestore into localStorage
            ...
            // Initialize the main application (only once per session)
            if (!appInitialized) {
                init();
                appInitialized = true;
            } else {
            ...
            // Set up real-time sync so other managers' changes appear instantly
            setupRealtimeSync();
        } else {
            currentUser = null;
            appInitialized = false;

            // Tear down real-time listeners
            activeSnapshotUnsubscribers.forEach(unsub => unsub());
            activeSnapshotUnsubscribers = [];
...
// Bootstrap auth UI when DOM is ready (handle case where DOMContentLoaded already fired)
...
// App initialization is now driven by Firebase Auth state (see setupAuthUI at top of file).
// init() is called after the user successfully authenticates.
